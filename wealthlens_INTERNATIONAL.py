...
# ── Razorpay Payment ──────────────────────────────────────────────────────────
RAZORPAY_KEY_ID = "rzp_live_SJGNjHCLfqEdeQ"

def razorpay_button(plan_name, amount_inr, plan_desc, color="#1a56db"):
    import streamlit.components.v1 as components
    uname  = st.session_state.get("user_name", "").replace('"', '')
    uemail = st.session_state.get("user_email", "").replace('"', '')
    fn     = f"rzpPay_{plan_name.replace(' ','').replace('/','_')}_{amount_inr}"
    html = f"""
    <script src="https://checkout.razorpay.com/v1/checkout.js"></script>
    <div id="msg_{amount_inr}" style="margin-bottom:6px"></div>
    <button onclick="{fn}()" style="
        background:{color};color:#ffffff;font-family:'DM Sans',sans-serif;
        font-weight:700;font-size:0.92rem;border:none;border-radius:10px;
        padding:0.8rem 1.5rem;cursor:pointer;width:100%;
        box-shadow:0 4px 14px rgba(0,0,0,0.18);
        transition:opacity 0.2s;letter-spacing:0.2px"
        onmouseover="this.style.opacity='0.88'"
        onmouseout="this.style.opacity='1'">
        💳 Pay ₹{amount_inr:,} — Secure Checkout
    </button>
    <script>
    function {fn}() {{
        var options = {{
            ...
            modal: {{
                ondismiss: function() {{
                    document.getElementById("msg_{amount_inr}").innerHTML =
                        "<span style='color:#f59e0b;font-size:0.8rem'>Payment cancelled. Click again when ready.</span>";
                }}
            }},
            handler: function(response) {{
                document.getElementById("msg_{amount_inr}").innerHTML =
                    "<div style='background:#f0fdf4;border:1px solid #86efac;border-radius:8px;padding:10px;margin-top:6px'>" +
                    "<div style='color:#16a34a;font-weight:700;font-size:0.88rem'>✅ Payment Successful!</div>" +
                    "<div style='color:#475569;font-size:0.78rem;margin-top:4px'>ID: " + response.razorpay_payment_id + "</div>" +
                    "<div style='color:#475569;font-size:0.78rem'>We will activate your account within 2 hours. Check your email.</div>" +
                    "</div>";
            }}
        }};
        ...
        rzp.on("payment.failed", function(response) {{
            document.getElementById("msg_{amount_inr}").innerHTML =
                "<div style='background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:10px;color:#dc2626;font-size:0.8rem'>" +
                "❌ Payment failed: " + response.error.description + ". Please try again.</div>";
        }});
        rzp.open();
    }}
    </script>
...
with tab1:
    ...
    for stock in portfolio:
        ...
        if len(series) > 50:
            ...
            series_s = series.dropna().sort_index()
            total_r = (series_s.iloc[-1] / series_s.iloc[0] - 1) * 100 if len(series_s) > 1 and series_s.iloc[0] > 0 else 0.0
...
with tab3:
    ...
    with col_b1:
        ...
        for stock in portfolio:
            name = stock['name']
            series = stock_data[name].dropna()
            if len(series) < 2:
                total_r = 0.0
            else:
                # Sort by index to ensure chronological order (oldest first)
                series_sorted = series.sort_index()
                start_price = series_sorted.iloc[0]
                end_price   = series_sorted.iloc[-1]
                # Sanity check: both prices must be positive
                if start_price > 0 and end_price > 0:
                    total_r = (end_price / start_price - 1) * 100
                else:
                    total_r = 0.0
    ...
    live_display = [
        ...
        ('Nippon India ETF Gold BeES', '₹126.00', '+0.31'),
...
with st.expander("🔐 Admin — View Email List & Leads"):
    ...
    if admin_pass == "wealthlens2026":
        ...
        Razorpay live payments are active. Users can pay via UPI, Card, or NetBanking.<br><br>
