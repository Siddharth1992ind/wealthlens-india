...
def subscription_section(portfolio_total=2000000):
    ...
    with c1:
        st.markdown("""<div style="background:#1a2235;border:1px solid #f5c842;border-radius:16px;padding:1.2rem;text-align:center">
    ...
    with c2:
        st.markdown("""<div style="background:#1a2235;border:2px solid #22c55e;border-radius:16px;padding:1.2rem;text-align:center;position:relative">
    ...
    with c3:
        st.markdown("""<div style="background:#1a2235;border:2px solid #a855f7;border-radius:16px;padding:1.2rem;text-align:center;position:relative">
...
if not st.session_state.free_user_registered:
    st.markdown("""
    <div style="background:linear-gradient(135deg,#0a0f1e,#111827);border:2px solid #f5c842;
                border-radius:20px;padding:2.5rem;text-align:center;margin-bottom:2rem">
        <div style="font-size:3rem">📊</div>
        <h2 style="color:#f5c842;font-family:'Playfair Display',serif;margin:0.5rem 0">
            Get Free Access to WealthLens India
        </h2>
        <p style="color:#94a3b8;font-size:1rem">
            Institutional-grade portfolio analysis — completely free.<br>
            Enter your details below to unlock all 5 screens instantly.
        </p>
        <div style="display:flex;justify-content:center;gap:2rem;margin:1rem 0;flex-wrap:wrap">
            <span style="color:#22c55e;font-size:0.85rem">✅ Monte Carlo simulation</span>
            <span style="color:#22c55e;font-size:0.85rem">✅ 5-year backtesting</span>
            <span style="color:#22c55e;font-size:0.85rem">✅ Rebalancing alerts</span>
            <span style="color:#22c55e;font-size:0.85rem">✅ Life stage planner</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.form("reg_form"):
        ...
        f3, f4 = st.columns(2)
        with f3:
            reg_phone = st.text_input("Mobile Number", placeholder="9876543210")
        with f4:
            reg_source = st.selectbox("How did you hear about us?",
                ["-- Select --","WhatsApp","LinkedIn","Twitter/X","Friend/Family","Google Search","YouTube","Other"])
        st.markdown('<div style="color:#64748b;font-size:0.75rem">By registering you agree to receive market updates. We never share your data.</div>', unsafe_allow_html=True)
        submitted = st.form_submit_button("🚀 Get Free Access Now", use_container_width=True)
        if submitted:
            if not reg_name or not reg_email or "@" not in reg_email:
                ...
                st.success(f"Welcome {reg_name}! 🎉 Full access unlocked.")
                st.rerun()

    st.markdown("""
    <div style="display:flex;justify-content:center;gap:3rem;margin-top:1rem;flex-wrap:wrap">
        <div style="text-align:center"><div style="color:#f5c842;font-size:1.5rem;font-weight:700;font-family:'Playfair Display',serif">500+</div><div style="color:#64748b;font-size:0.8rem">Portfolios Analysed</div></div>
        <div style="text-align:center"><div style="color:#f5c842;font-size:1.5rem;font-weight:700;font-family:'Playfair Display',serif">₹12Cr+</div><div style="color:#64748b;font-size:0.8rem">Assets Tracked</div></div>
        <div style="text-align:center"><div style="color:#f5c842;font-size:1.5rem;font-weight:700;font-family:'Playfair Display',serif">CFA-Grade</div><div style="color:#64748b;font-size:0.8rem">Research Framework</div></div>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
